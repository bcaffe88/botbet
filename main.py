...
# MAPA DE CONFIANÇA ATUALIZADO PARA SUPORTAR NOTA 9.0
CONFIDENCE_MAP = {
    "MÉDIA-ALTA": f"MÉDIA-ALTA ⚡ STAKE {MEDIA_ALTA_STAKE}",
...
async def analisar(texto):
    ...
    try:
        ...
        if "U20" in jogo.upper() or "U19" in jogo.upper(): 
            logger.info(f"🚫 Jogo ignorado (Filtro Sub-19/20): {jogo}")
            return
        ...
        logger.info(f"📈 Téc: {pontos_tecnicos} | Clima: {pontos_clima} | Total: {pontos_total} | Jogo: {jogo}")

        # =========================================================
        # APENAS A SUA LÓGICA DE APROVAÇÃO DAS 3 CONDIÇÕES AQUI
        # =========================================================
        ...
        if deve_entrar:
            ...
            logger.info(f"❌ Pontuação insuficiente para aprovar '{jogo}'. (Total: {pontos_total} | Téc: {pontos_tecnicos} | Clima: {pontos_clima})")
